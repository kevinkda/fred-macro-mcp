"""Shared runtime helper for the business tool modules.

Each business tool needs:

1. A lazily-constructed :class:`FredClient` (so tests can override the
...
from ..client import FredClient, make_client
...
async def get_client() -> FredClient:
    """Lazily instantiate the FRED client (one per process)."""
    global _client
    if _client is None:
        async with _lock:
...
async def set_client_for_tests(client: FredClient | None) -> None:
...
async def call_with_cache(
    fetch: Callable[[FredClient], Awaitable[dict[str, Any]]],
    *,
