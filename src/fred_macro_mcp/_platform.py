...
Mirrors the sec-edgar-mcp ``_platform`` shim so all sibling MCP servers
share the same platform-neutral surface area.
"""
...
log = logging.getLogger("fred_macro_mcp._platform")
...
def _notify_windows(title: str, message: str) -> None:  # pragma: no cover - windows-only branch
    ...
    try:
        ...
        notification.notify(title=title, message=message, app_name="FRED Macro MCP", timeout=5)
        return
    ...
    script = (
        ...
        '[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("FRED Macro MCP")'
