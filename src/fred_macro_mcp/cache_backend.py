"""Pluggable cache backends for fred-macro-mcp (v0.7 T0 template).
...
* :class:`MemoryBackend` — the **default**, zero-external-dependency
  ...
  event loop.  Time-series operations return a structured
...
* :class:`ClickHouseBackend` — the **opt-in** backend.  Enabled only when
  ``pip install fred-macro-mcp[clickhouse]`` is present *and*
  ``FRED_CLICKHOUSE_URL`` is configured.  ``clickhouse_connect`` is
...
* :func:`get_cache_backend` — the factory.  Selects the backend from
  ``FRED_CACHE_BACKEND`` (``memory`` | ``clickhouse``, default ``memory``).
...
ENV_CACHE_BACKEND: Final[str] = "FRED_CACHE_BACKEND"
ENV_CLICKHOUSE_URL: Final[str] = "FRED_CLICKHOUSE_URL"
...
REQUIRES_CLICKHOUSE_HINT: Final[str] = (
    ...
    "Install it with `pip install fred-macro-mcp[clickhouse]` and set "
    "FRED_CLICKHOUSE_URL plus FRED_CACHE_BACKEND=clickhouse. "
...
def requires_clickhouse_signal() -> dict[str, Any]:
    ...
    analysis degrades gracefully instead of raising.  Core response-cache
    tools never see this.
    """
...
class CacheBackend(Protocol):
    ...
    * **Derived-analysis history** (``append_timeseries`` /
      ``query_timeseries``) — persist time series.  Only the ClickHouse
      backend persists; the memory backend returns a
      ``requires_clickhouse_persistence`` signal.
    """
...
class ClickHouseNotInstalledError(RuntimeError):
    ...
    def __init__(self, original: Exception | None = None) -> None:
        self.hint = (
            ...
            "`pip install fred-macro-mcp[clickhouse]`."
        )
...
def _import_clickhouse_connect() -> Any:
    ...
    return clickhouse_connect  # pragma: no cover - reached only with the optional `clickhouse` extra installed
...
class ClickHouseBackend:
    ...
    ``FRED_CLICKHOUSE_URL`` is configured.  Connection / query failures
    degrade best-effort (response cache misses, time-series returns a
    degradation signal) so a flaky ClickHouse never breaks a core tool.
    """

    name = "clickhouse"

    _RESPONSE_TABLE: Final[str] = "fred_response_cache"
    _TIMESERIES_TABLE: Final[str] = "fred_timeseries"
...
def get_cache_backend() -> CacheBackend:
    ...
    Honors ``FRED_CACHE_BACKEND`` (``memory`` | ``clickhouse``).  Any
    unrecognised value falls back to ``memory`` — the zero-dependency
